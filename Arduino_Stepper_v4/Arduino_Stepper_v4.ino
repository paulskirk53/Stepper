...
int lower_limit = 0;         // just in case 'fawlty' data comes in from driver
int upper_limit = 360;
...
void setup()
{
  ...
  /*
    ...
    my setting:
    5     6    7    8
    on   on  off  on =  step angle ~ 0.25 degree this changed in Feb 2019 to set the smallest step size and max current value
...
void loop()
{
  ...
  if (Serial.available() > 0)                              // when serial data arrives capture it into a string
  {
    ...
    if (receivedData.startsWith("CL", 0))            // clockwise Slew command from C# driver
    {
      ...
      // used 300000 as one full rev of dome and this should therefore cover any size slew
    ...
    if (receivedData.startsWith("SL", 0)) // Receive the Current AZ via driver from encoder
    {
